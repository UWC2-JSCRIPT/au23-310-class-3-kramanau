...
const player1 = { name : 'Player 1', health : 10 };
const player2 = { name : 'Player 2', health : 10 };
...
attack = (attackingPlayer, defendingPlayer, baseDamage, variableDamage) => {
  const totalDamage = baseDamage + Math.floor(Math.random() * variableDamage);
  defendingPlayer.health -= totalDamage;
  return `${attackingPlayer.name} attacked ${defendingPlayer.name} for ${totalDamage} damage.`;
};
